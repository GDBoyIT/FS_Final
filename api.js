...
const url = "mongodb://localhost:27017/ppop";
// ===============================
...
function getProductById(req, res) {
    ...
    Product.find({ "id": pid }, function (err, products) {
...
function updateProductById(req, res) {
    ...
    Product.findByIdAndUpdate(pid, ayload, function (err) {
        if (err) res.status(500), json(err);
...
function addProduct(req, res) {
    ...
    product.save(function (err) {
        if (err) res.status(500).json(err);
        res.json({ status: "Addded aproduct" });
    });
    // ===============================
}
